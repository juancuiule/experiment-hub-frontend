...
describe("buildSchema", () => {
  describe("input", () => {
    it("passes when required input has a value", () => {
      const result = schema([{ type: "input", dataKey: "name", label: "Name", required: true }])
        .safeParse({ name: "Juan" });
    ...
    it("passes when optional input is empty", () => {
      const result = schema([{ type: "input", dataKey: "note", label: "Note" }])
        .safeParse({ note: "" });
      expect(result.success).toBe(true);
    });
  });
  ...
  describe("checkbox-group", () => {
    it("passes when required checkbox-group has selections", () => {
      const result = schema([{ type: "checkbox-group", dataKey: "tags", label: "Tags", required: true, options: [] }])
        .safeParse({ tags: ["a"] });
      expect(result.success).toBe(true);
    });

    it("fails when required checkbox-group is empty", () => {
      const result = schema([{ type: "checkbox-group", dataKey: "tags", label: "Tags", required: true, options: [] }])
        .safeParse({ tags: [] });
      expect(result.success).toBe(false);
    ...
    it("passes when optional checkbox-group has no selections", () => {
      const result = schema([{ type: "checkbox-group", dataKey: "tags", label: "Tags", options: [] }])
        .safeParse({ tags: [] });
      expect(result.success).toBe(true);
    });
  });
  ...
  describe("rating", () => {
    it("passes when required rating has a value", () => {
      const result = schema([{ type: "rating", dataKey: "score", label: "Score", scale: 5, required: true }])
        .safeParse({ score: "3" });
      expect(result.success).toBe(true);
    });
    ...
    it("passes when optional rating is null", () => {
      const result = schema([{ type: "rating", dataKey: "score", label: "Score", scale: 5 }])
        .safeParse({ score: null });
      expect(result.success).toBe(true);
    });
  });
  ...
  describe("non-data components", () => {
    it("ignores button components", () => {
      const result = schema([{ type: "button", label: "Submit" }]).safeParse({});
      expect(result.success).toBe(true);
    });

    it("ignores rich-text components", () => {
      const result = schema([{ type: "rich-text", content: "## Hello" }]).safeParse({});
      expect(result.success).toBe(true);
    });
  });
  ...
  describe("mixed required and optional fields", () => {
    it("fails only for the required field when optional is empty", () => {
      const result = schema([
        ...
        { type: "input", dataKey: "note", label: "Note" },
      ]).safeParse({ name: "", note: "" });
      expect(result.success).toBe(false);
